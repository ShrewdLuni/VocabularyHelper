...
export const MainPage = ({onPlay, categories,updateCategories,setAllCategories,gameMode,setGameMode} : MainPageProps) => {
    ...
    <div className="text-center bg-gray-900 h-screen flex flex-col justify-center items-center">
      <div className="grid grid-cols-4 gap-4 lg:gap-6 w-[90%] lg:w-[50%]">
        ...
        <Button className={cn("bg-rose-500 text-white w-full h-full text-lg",categories["routine"] && "border-solid border-green-500 border-2 p-[2px] lg:border-4 lg:p-[4px]")} variant="ghost" onClick={() => {updateCategories({...categories,"routine" : !categories["routine"]})}}>{"Routine"}</Button>
        ...
        <Button className={cn("bg-rose-500 text-white w-full h-full text-lg",categories["weather"] && "border-solid border-green-500 border-2 p-[2px] lg:border-4 lg:p-[4px]")} variant="ghost" onClick={() => {updateCategories({...categories,"weather" : !categories["weather"]})}}>{"Weather"}</Button>
        <Button className={cn("bg-rose-500 text-white w-full h-full text-lg",categories["location"] && "border-solid border-green-500 border-2 p-[2px] lg:border-4 lg:p-[4px]")} variant="ghost" onClick={() => {updateCategories({...categories,"location" : !categories["location"]})}}>{"Location"}</Button>
